...
def main():
    ...
    # 这个是专门弄3个点的代码
    # 这里如果使用自适应sigma，则在sigma_vector 中生成一个[3,2]的全1向量作为初始值
    if cfg.MODEL.INDEPENDENT_SIGMA:
        ...
        # 拉直合并两个list
        sigma_x_list = np.array(sigma_x_list).reshape(-1, 1) * cfg.MODEL.SIGMA
        sigma_y_list = np.array(sigma_y_list).reshape(-1, 1) * cfg.MODEL.SIGMA
        # sigma_vector = torch.from_numpy(np.hstack((sigma_y_list, sigma_x_list)))
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        print('sigma_vector value is {}'.format(sigma_vector))
